...
voiceover-v3 上「叙事短片 / 解说纪录片」是被提到页面层的生成器参数 —— **它只决定 `video_type`，不切换任何输入框**。页面只有**一个 textarea**，两种视频类型**共用同一份文本**（存在 `script`）。选解说纪录片时把它镜像进 `subtitleInput` —— 那份文本本身就是字幕/解说词。镜像发生在两处：日常输入走 `setConceptText`，切换到解说纪录片那一刻由一个 effect 补一次（该 effect 只认类型变化，不能跟着 `script` 跑，否则会把分镜导入写进 `subtitleInput` 的字幕覆盖掉）。切回叙事短片不清空 `subtitleInput`。

**字幕管线本身没动** —— `/voiceover/init` 入参、`ttsScript`、落库的 `subtitle_input`、时长估算全部照旧读 `subtitleInput`，只是它的来源从自己的输入框换成了共享框。下方「配音（可选）」只剩音色选择和 TTS。「专业分镜生成」按钮在「视频概念描述」标题行右侧，点开是**浮窗**（经 `createPortal` 挂到 `body`，避开页面的 sticky 头部和 overflow 容器；遮罩层透明只用来接外部点击，不遮挡也不锁页面滚动；Esc / 点外部关闭）。
