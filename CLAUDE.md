...
6. **分镜视频生成** — Submit each shot to Seedance API for video generation
   ...
   - **分镜卡的「提示词」页签是分段显示的**（`splitShotPrompt` / `joinShotPrompt`，page.tsx）：
     一条 `prompt_en` 其实由四段拼成，来源和「谁说了算」各不相同 ——
     **开场声明**（模型写的格式/风格声明）、**角色定义**（`将@图片N中<外貌原文>定义为<主体N>；`
     固定句式，后端逐镜统一贴）、**画面描述**（真正属于这一镜的场景/动作/节拍）、
     **台词**（`appendSpeech` 追加的对白/画外音块，提交时按字幕重建）。
...
- **只定义这一镜真的出场的角色**：出场与否按「定义句之外还提没提到 `<主体N>` / `@图片N`」判定，
  **不采信模型写了几句定义** —— 角色单是整片共用的，模型习惯性地每镜把所有人都定义一遍，
  多贴的那句等于告诉 Seedance「画面里还有这么一个人」，轻则挤进背景，重则两张脸揉到一起。
  兜底：正文一个标签都没提、模型却确实定义了人时，退回模型定义的那批（宁可多贴也不要不贴）
