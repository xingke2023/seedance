...
**字幕管线本身没动** —— `/voiceover/init` 入参、`ttsScript`、落库的 `subtitle_input`、时长估算全部照旧读 `subtitleInput`，只是它的来源从自己的输入框换成了共享框。下方「配音（可选）」（音色选择 + TTS）**只在解说纪录片下出现** —— 叙事短片的字幕为空，没有可配音的文本。「专业分镜生成」按钮在「视频概念描述」标题行右侧，点开是**浮窗**（经 `createPortal` 挂到 `body`，避开页面的 sticky 头部和 overflow 容器；遮罩层透明只用来接外部点击，不遮挡也不锁页面滚动；Esc / 点外部关闭）。
