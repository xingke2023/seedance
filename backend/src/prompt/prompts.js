...
## 核心要求：旁白与对白穿插着讲这个故事
1. **两种声音各司其职，按镜交替使用**：
   - **对白**（type=dialogue）：画面里有人、脸看得清 —— 写成他开口说出来的话，
     承担人物关系、冲突、态度这些「当场发生」的信息
   - **旁白**（type=narration）：空镜、转场、时间跳跃、交代前情或收束主题 ——
     写成画外音，承担对白说不出口的信息（心里话、时间流逝、背景交代）
   全片旁白大约占四分之一到三分之一；开场可以用一句旁白起势，结尾可以用一句旁白收束，
   **中间不要连着两镜都是旁白**，也不要通篇只有对白 —— 只有对白会像舞台剧，只有旁白会像解说片
2. **一个镜头里一般只用一种**。确实需要两种时：旁白在前、对白在后，
   两段加起来仍不能超过该镜时长（旁白起个头，人物接话）
3. 对白要像真人在说话：口语、短句、有来有回。一个镜头里可以是两个角色对答（最多 3 句）
4. **先定音色，再写台词**：在 voices 里为每个开口说话的角色写一句英文音色描述
   —— **画外旁白也算一个说话人**（speaker 写「旁白」），它同样会被念出声，也要给它一句音色描述
...
6. 字数与镜头时长匹配：约每秒 3.5 个字（5s≈17字，8s≈28字，10s≈35字）。
   一个镜头里所有台词加起来不能超过该镜头时长
7. **短镜头的台词要更短**：4 秒以内的镜头，单句不超过 10 个字 —— 台词一长，口型就会漂
8. 通篇连贯 —— 所有镜头的台词合起来是一个完整故事，不是各镜头互不相干的说明
9. 直接写说出口的内容，不要加「某某说：」这类提示语；不要复述画面里已经看得见的东西
10. **语种统一**：一句台词里不要中英混用（专有名词除外）—— 混用会让模型的发音在中途切换
11. **多音字、生僻字、形近字要换掉**：模型经常读错，改写成发音一致的常用同音字
...
两种都会被念出声、也都会烧成字幕，所以字数一样要卡住该镜时长；
同一个镜头里对白和旁白加起来不能超时。
重要：字符串值内禁止使用英文双引号 "，需引用时请改用 「」或（）。`
