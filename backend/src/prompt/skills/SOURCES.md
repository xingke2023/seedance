...
| **台词语种统一 + 中文发音兜底**（多音字改同音字） | `SCRIPT_SYSTEM`「台词是要被念出来的」1-2 | 混用语种会中途切发音；多音字/生僻字模型常读错 |
...
`STORYBOARD_SYSTEM`/`QCZH_SYSTEM` 共用的 `shotSplitDialogueRules()`（`prompts.js`）里的
**音色条件句**（每个角色一句 `voice_en`，逐镜原样粘贴）来自 `seedance-2-5/SKILL.md` 的
Voice 一节 —— 与版本无关的通用规律：我们每个分镜是一次独立生成，不锁音色，
同一个角色逐镜声音都不一样。这条规则原来写在已经删掉的 `DIALOGUE_SYSTEM` 里，
叙事短片改成「先写剧本再分镜配运镜」两步式之后随对白分配的活儿一起搬进了这两个提示词。
